...
public class DefaultServiceDiscoveryTest {
	...
	private static final Scheme HTTP = new Scheme("http",false,80);
	...
	public void start() {
		...
		discovery.addService(new BackendService("example.com", null, null,null,false, null,Collections.singletonList(new Endpoint(HTTP,new TcpAddress("127.0.0.1",8080)))));
		discovery.addService(new BackendService("example.com", null, "/resource",null,false,null,Collections.singletonList(new Endpoint(HTTP,new TcpAddress("127.0.0.2",8080)))));
		discovery.addService(new BackendService("example2.com", null, "/",null,false,null,Collections.singletonList(new Endpoint(HTTP,new TcpAddress("127.0.0.3",8080)))));
		discovery.addService(new BackendService("example2.com", null, "/more/specific/path",null,false,null,Collections.singletonList(new Endpoint(HTTP,new TcpAddress("127.0.0.4",8080)))));
	}
	...
	public void matchOnlyHostSpecificService() throws InterruptedException, ExecutionException {
		...
		assertThat(((TcpAddress)endpoint.address()).host()).isEqualTo("127.0.0.1");
	...
	public void matchHostMethodAndPathSpecificService() throws InterruptedException, ExecutionException {
		...
		assertThat(endpoint).isNotNull();
		assertThat(((TcpAddress)endpoint.address()).host()).isEqualTo("127.0.0.2");
	...
	public void matchMoreSpecificPathService() throws InterruptedException, ExecutionException {
		...
		assertThat(endpoint).isNotNull();
		assertThat(((TcpAddress)endpoint.address()).host()).isEqualTo("127.0.0.4");
	}
