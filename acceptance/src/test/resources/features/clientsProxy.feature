...
Scenario Outline: Client using <type> proxy
	...
	When I get url "http://localhost:8080/responsecode/204" through proxy "#proxy" getting response "#response"
...
Scenario Outline: Client using proxy with authentication success
	...
	When I start a service "#serviceFuture" with API "com.simplyti.service.examples.api.APITest"
	Then I check that "#serviceFuture" is success
	When I get url "http://localhost:8080/responsecode/204" through proxy "#proxy" with username "proxyuser" and password "123456" getting response "#response"
	Then I check that "#response" is success
	And I check that http response "#response" has status code 204
	And I check that http response "#response" has body ""
	Examples:
