...
public class DefaultNamespaces extends DefaultK8sApi<Namespace> implements Namespaces {
	...
	public DefaultNamespaces(EventLoopGroup eventLoopGroup, HttpClient http) {
	...
	private Status response(FullHttpResponse response) {
		...
		return JsonIterator.deserialize(data,Status.class);
	}

	@Override
	public NamespacesBuilder builder() {
		return new DefaultNamespacesBuilder(http(), K8sAPI.V1);
	}
