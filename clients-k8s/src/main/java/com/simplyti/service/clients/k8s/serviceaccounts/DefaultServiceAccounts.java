...
public class DefaultServiceAccounts extends DefaultK8sApi<ServiceAccount> implements ServiceAccounts {
	...
	public DefaultServiceAccounts(EventLoopGroup eventLoopGroup, HttpClient http) {
	...
	public NamespacedServiceAccounts namespace(String namespace) {
		return new DefaultNamespacedServiceAccounts(eventLoopGroup(),http(),K8sAPI.V1,RESOURCE,LIST_TYPE,EVENT_TYPE,namespace);
	}

	@Override
	public ServiceAccountBuilder builder(String namespace) {
		return new DefaultServiceAccountBuilder(http(), K8sAPI.V1, namespace, RESOURCE);
	}
